<template>
	<div class="page-login">
		...
		<!--<div class="tm-show">{{time}}</div>-->
		
		<div class="right">
			
			<el-carousel :interval="4000" style="height:100%;">
				<el-carousel-item v-for="(item,index) in bg.length" :key="item">
...
<script>
	...
	import Vue from 'vue'
	import {Carousel,CarouselItem,Input,Form,FormItem} from 'element-ui';
...
//			]),
			...
			submit () {
				...
				this.$refs.loginForm.validate((valid) => {
					if (valid) {
						this.$emit("login",Object.assign({},this.formLogin));
						// 登录
						// 注意 这里的演示没有传验证码
						// 具体需要传递的数据请自行修改代码
//						this.login({
