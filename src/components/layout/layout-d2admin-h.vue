<template>
  <div  :class="themeClassWrap" style="width: 100%;
    ...
    position: relative;">
    <div
      class="d2-layout-horizontal-aside-group "
      ...
      <div class="d2-layout-header-aside-content" flex="dir:top" >
        ...
        <div class="d2-theme-container" flex-box="1" flex>
          ...
          <div class="d2-theme-container-aside"
            ...
            <!--todo:菜单插槽-->
            <slot name="menusider"  flex-box="1"></slot>

          </div>

          <!-- 主体 -->
          <div class="d2-theme-container-main" flex-box="1" flex="dir:top main:left">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pageClose(pages){
        pages.forEach((item) => {
          ...
          //关闭的时候把地址重置
          var url = this.iframeList[index].url.split('?rom=')[0];
          this.iframeList[index].url = url +'?rom='+Math.random();
        });
      },
      /**
       * 添加页面
      ...
      refreshPage(){
        ...
        let index = this.iframeList.findIndex((tempObj) => {
          return tempObj.id == this.iframectivate;
        })
        //关闭的时候把地址重置
        var url = this.iframeList[index].url.split('?rom=')[0];
        this.iframeList[index].url = url +'?rom='+Math.random();
      ...
      // API 刷新页面
      apiRefresh(){
        this.$amui.$emit(eventConst.REFRESH_PAGE_EVENT);
      }
    },
    ...
    mounted(){
      ...
      this.$amui.$on(eventConst.PAGE_ADD_EVENT,this.pageAdd);
      ...
      this.$amui.$on(eventConst.REFRESH_PAGE_EVENT,this.refreshPage);
    }
  }
</script>
