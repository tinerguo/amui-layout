<template>
	<div class="tab-d2admin d2-multiple-page-control-group" flex>
		
		<div class="d2-multiple-page-control-content" flex-box="1" style="background: rgba(255,255,255,0.8);">
		...
		methods: {
			...
			add (newTag) {
				...
				if (this.opened.map(x => x.name).indexOf(newTag.name) === -1) {
					...
					this.$amui.$emit(eventConst.PAGE_ADD_EVENT, newTag);
				}else{
					this.$amui.$emit(eventConst.PAGE_CHANGE_EVENT, newTag);
				}
			...
			close({tagName, vm}){
				...
				if (index >= 0) {
					// 触发页面关闭事件
					this.$amui.$emit(eventConst.PAGE_CLOSE_EVENT, [this.opened[index]]);
			...
			closeLeft(params){
				...
				let arr = this.opened.slice(0, currentIndex);
				// 触发页面关闭事件
				this.$amui.$emit(eventConst.PAGE_CLOSE_EVENT, arr);
			...
			closeRight(params){
				...
				let arr = this.opened.slice(currentIndex + 1);
				// 触发页面关闭事件
				this.$amui.$emit(eventConst.PAGE_CLOSE_EVENT, arr);
			...
			closeOther(params){
				...
				if (currentIndex === 0) {
					...
					// 触发页面关闭事件
					this.$amui.$emit(eventConst.PAGE_CLOSE_EVENT, this.opened.slice(1));
					this.opened.splice(1)
				} else {
					var arr1 = this.opened.slice(currentIndex + 1);
					var arr2 = this.opened.slice(0, currentIndex);
					this.$nextTick(function(){
						this.$amui.$emit(eventConst.PAGE_CLOSE_EVENT, Object.assign([],arr1.concat(arr2))  );
					});
