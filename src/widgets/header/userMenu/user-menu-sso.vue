<template>
	...
	<div class="user-menu" style="margin:0 22px;color:#fff;font-size: 14px;margin-top:-4px;">
		...
		<template v-if="!userName">
			...
			<a  :href="''+LOGIN_URL" style="color:#fff;">请登录</a>
		</template>
	...
	export default {
		...
		data () {
			const checkPwdConn = (rule, value, callback) => {
				...
				var getData = {};
				http({
					url: window.configs.AUUM_ROOT + 'v1/Users/{uname}/{password}/getPassword',
					method: 'get',
					requireAuth: true,
					params: {
			...
			return {
				...
				personObj:{},
				posList:[],
				LOGIN_URL:window.configs.LOGIN_URL
			}
		},
		filters:{
			getGender(value){
		...
		methods: {
			async getUser(){
				...
				//通过网关获取
				var res = await http({
					url:window.configs.API_ROOT+'loginuser',
					method: 'get'
				});
				this.userName = res;
				
				var res2 = await http({
					url:window.configs.AUUM_ROOT+'v1/Users/'+this.userName+'/getUserInfo',
					head:'raw',
					method: 'post'
				});
				...
				var res3 = await http({
					url:window.configs.AUUM_ROOT+'v1/Persons/'+this.idcode,
					method: 'get'
				});
				this.personObj = res3;
				...
				var res4 = await http({
					url:window.configs.AUUM_ROOT+'/v1/Users/'+this.userName+'/pos',
					method: 'get'
				});
				this.posList = res4;
			...
			async sendUpdatePwd(){
				...
				var res = await http({
					url: window.configs.AUUM_ROOT + 'v1/Users/changePassword',
					head: 'raw',
					method: 'post',
					data: {
						idcode: this.idcode,
		...
		mounted(){
			if(!window.configs.API_ROOT){
				this.$Message.error("用户组件提示:请配置API_ROOT!");
			}
			if(!window.configs.AUUM_ROOT){
