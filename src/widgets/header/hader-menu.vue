<template>
	<Menu ref="myMenu" mode="horizontal" :active-name="currentName" style="background: rgba(0,0,0,0);border:none;margin-right:60px;">
		<template v-for="submenu in getMenuData" title="" name="">
			
			<MenuItem v-if="submenu.children.length == 0" @click.native="handleMenuClick(submenu)"
			          :name="submenu.title" :key="submenu.id">
				<i style="font-size: 22px;width: 22px;" v-if="submenu.icon"
				   :class="`${submenu.icon}`"></i>
				<d2-icon-svg v-else-if="submenu.iconSvg" :name="submenu.iconSvg"/>
				<!-- <i v-else class="fa fa-file-o"></i> -->
				
				<span v-if="submenu.target" :title="submenu.title"><a :href='submenu.url'
				                                                      target='_blank'>{{submenu.title}}</a></span>
