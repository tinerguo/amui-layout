/**
 * Created by tiner on 2018/11/23.
 */
import Vue from 'vue';
...
/** tiner create 2018-9-10
 https://www.cnblogs.com/tugenhua0707/p/9275958.html
 var temp = {
  id: 'unitcode',//数据唯一值字段名称
  parendId: 'punitcode',//父id字段名称
  name: 'unitname',//
  rootId: null,//顶级数据数据值
  childrenParamName:'child',//子列表属性名称 :child []
  expandFun:function(){//过滤器
    return true;
  }
};
 **/
...
/** 数据过滤 **/
export function fileData(arr,paramsObj,addParams){
  var r_arr = [];
  for(var i =0;i<arr.length;i++){
    var obj = arr[i];
    for(var key in paramsObj){
      obj[paramsObj[key]] =  obj[key];
    }
...
export function fileDatakey(arr,keyArr){
  var r_arr = [];
  for(var i =0;i<arr.length;i++){
    var obj = arr[i];
    obj["key"] = "";
